...
// The expensive mistake: charging cache reads as fresh input. At Sonnet rates
// ($3/M in) 1M cache-read tokens cost $0.30, not $3.00.
test('cache reads bill at a tenth of fresh input', () => {
  ...
  assert.ok(Math.abs(freshCost - 3) < 1e-6, `expected $3.00 for fresh input, got ${freshCost}`);
  assert.ok(Math.abs(cachedCost - 0.3) < 1e-6, `expected $0.30 for cache reads, got ${cachedCost}`);
});
