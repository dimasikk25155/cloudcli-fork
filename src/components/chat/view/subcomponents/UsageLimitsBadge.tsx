 ...
 * Polls /api/usage/limits once a minute; tapping it opens a small panel with
 * reset times (Moscow) and the run-guard threshold.
 */
export default function UsageLimitsBadge() {
  ...
  const fiveHourPct = usage?.fiveHourPct ?? null;
  const sevenDayPct = usage?.sevenDayPct ?? null;
  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        ...
        title="Лимиты подписки Claude"
        aria-label="Показать лимиты подписки"
      >
        <span className="grid h-5 w-5 place-items-center rounded-md bg-primary/10 text-primary">
          <GaugeIcon className="h-3.5 w-3.5" />
        </span>
        <span className={`font-medium ${pctTone(fiveHourPct)}`}>5ч {pctLabel(fiveHourPct)}</span>
      ...
      {open && (
        <div className="absolute bottom-full left-0 z-50 mb-2 w-64 rounded-lg border border-border bg-popover p-3 text-xs text-popover-foreground shadow-lg">
          <div className="mb-2 font-semibold">Лимиты подписки Claude</div>
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">5-часовое окно</span>
              <span className={`font-medium ${pctTone(fiveHourPct)}`}>{pctLabel(fiveHourPct)}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">сброс</span>
              <span>{formatResetMoscow(usage?.fiveHourResetsAt ?? null)}</span>
            </div>
            ...
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">сброс</span>
              <span>{formatResetMoscow(usage?.sevenDayResetsAt ?? null, true)}</span>
            </div>
            <div className="my-1.5 border-t border-border/60" />
            <div className="text-muted-foreground">
              При ≥{usage?.threshold ?? 90}% пятичасового окна новые прогоны придерживаются — остаток
              зарезервирован под личные вопросы. Обновляется раз в минуту.
            </div>
          </div>
        </div>
      )}
