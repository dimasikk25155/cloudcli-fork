...
import {
  computeMerged,
  dropStopHookUserMessages,
  isStopHookUserMessage,
...
export function useSessionStore() {
  ...
  const fetchFromServer = useCallback(async (
    ...
    try {
      ...
      const messages: NormalizedMessage[] = dropStopHookUserMessages(data.messages || []);
      ...
      slot.status = 'idle';
      // Same prune as refreshFromServer: a stale refetch mid-turn used to
      // keep live rows whose text already landed in JSONL under new ids.
  ...
  const fetchMore = useCallback(async (
    ...
    try {
      ...
      const body = await response.json();
      const data = body?.data ?? body;
      const olderMessages: NormalizedMessage[] = dropStopHookUserMessages(data.messages || []);
  ...
  const appendRealtime = useCallback((sessionId: string, msg: NormalizedMessage) => {
    ...
    // A new prompt must not keep the previous answer's live row sitting above
    // it, and must not let a later replay of that row attach after the prompt.
    if (normalizedMessage.kind === 'text' && normalizedMessage.role === 'user') {
      slot.realtimeMessages = pruneRealtimeSupersededByServer(
        slot.serverMessages,
        slot.realtimeMessages,
      );
  ...
  const appendRealtimeBatch = useCallback((sessionId: string, msgs: NormalizedMessage[]) => {
    const incoming = dropStopHookUserMessages(msgs);
    ...
    slot.realtimeMessages = updated;
    if (normalizedMessages.some((message) => message.kind === 'text' && message.role === 'user')) {
      slot.realtimeMessages = pruneRealtimeSupersededByServer(
        slot.serverMessages,
        slot.realtimeMessages,
      );
  ...
  const refreshFromServer = useCallback(async (
    ...
    try {
      ...
      slot.serverMessages = dropStopHookUserMessages(data.messages || []);
  ...
  const updateStreaming = useCallback((sessionId: string, accumulatedText: string, msgProvider: LLMProvider) => {
    ...
    if (idx >= 0) {
      const laterUser = slot.realtimeMessages.slice(idx + 1).some(
        (row) => row.kind === 'text' && row.role === 'user',
      );
